...
func client() {
	...
	// get menu (request)
	resp, err := http.Get("http://localhost:8011/menu")
	if err != nil {
		log.Fatal(err)
	}
	...
	resp, err = http.Post("http://localhost:8011/order", "application/json", bytes.NewBuffer(json_data))
	if err != nil {
		log.Fatal(err)
	}
...
func make_review(restaurant_id int, receivedOrder models.OrderRegistered) {
	...
	resp, err := http.Post("http://localhost:8011/rating", "application/json", bytes.NewBuffer(json_data))
	if err != nil {
		log.Fatal(err)
	}
